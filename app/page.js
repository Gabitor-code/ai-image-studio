...
export default function Home() {
  ...
  const settingsSummary = isVideoLike ? `${mode === 'video' && videoTier === 'cinematic' ? 'Cinematic' : videoResolution} · ${duration}s` : `${resolution}px · ${quality === 'standard' ? 'Standard' : 'High detail'}`;
  ...
  const creditCost = isVideoLike
    ? (mode === 'video' && videoTier === 'cinematic' ? 4 : (videoResolution === '1080p' ? 8 : 5)) * (([5, 8, 10].includes(Number(duration)) ? Number(duration) : 5))
    : 2;
