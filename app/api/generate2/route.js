...
export async function POST(request) {
  try {
    ...
    const input=referenceImage?{prompt:finalPrompt,image_path:referenceImage,seed:Math.floor(Math.random()*999999999999999),guidance:2.5,width:size,height:size}:{workflow:{'6':{inputs:{text:finalPrompt,clip:['30',1]},class_type:'CLIPTextEncode'},'8':{inputs:{samples:['31',0],vae:['30',2]},class_type:'VAEDecode'},'9':{inputs:{filename_prefix:'Gabitor',images:['8',0]},class_type:'SaveImage'},'27':{inputs:{width:size,height:size,batch_size:1},class_type:'EmptySD3LatentImage'},'30':{inputs:{ckpt_name:'flux1-dev-fp8.safetensors'},class_type:'CheckpointLoaderSimple'},'31':{inputs:{seed:Math.floor(Math.random()*999999999999999),steps:quality==='standard'?16:28,cfg:1,sampler_name:'euler',scheduler:'simple',denoise:1,model:['30',0],positive:['35',0],negative:['33',0],latent_image:['27',0]},class_type:'KSampler'},'33':{inputs:{text:'',clip:['30',1]},class_type:'CLIPTextEncode'},'35':{inputs:{guidance:3.5,conditioning:['6',0]},class_type:'FluxGuidance'}}};
    const target=referenceImage?editEndpoint:endpoint;
    const response=await fetch('https://api.runpod.ai/v2/'+target+'/runsync',{method:'POST',headers:{'Content-Type':'application/json',Authorization:'Bearer '+apiKey},body:JSON.stringify({input})});
    ...
    return Response.json({image:image.startsWith('data:')||image.startsWith('http')?image:'data:image/png;base64,'+image,remainingCredits:creditData?.remaining_credits});
